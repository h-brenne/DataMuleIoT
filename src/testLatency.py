...
rdn = RaidenAPIWrapper(ip=URL, port=PORT2)
rdn2 = RaidenAPIWrapper(ip=URL_RPI_ZERO, port=PORT_RPI)

latencies = []
for i in range(5):
    latencies.append(ping_pong_transaction(rdn, rdn2, PARTNER4, PARTNER_RPI_ZERO, TOKEN_ADDR))
analyze_ping_pong_transactions(latencies)

with open("latencies_rpi_5_mediating_0_loss_correct", 'wb') as out:
